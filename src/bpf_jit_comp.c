...
static inline _Bool is_imm8(int value)
{
    return value <= 127 && value >= -128;
}

static inline _Bool is_simm32(__s64 value)
{
 ...
 */
static inline _Bool is_ereg(__u32 reg)
{
...
static inline __u8 add_1mod(__u8 byte, __u32 reg)
{
    if (is_ereg(reg))
        byte |= 1;
    return byte;
...
static inline __u8 add_2mod(__u8 byte, __u32 r1, __u32 r2)
{
    if (is_ereg(r1))
        byte |= 1;
    if (is_ereg(r2))
...
static inline __u8 add_1reg(__u8 byte, __u32 dst_reg)
{
    return byte + reg2hex[dst_reg];
}

/* encode 'dst_reg' and 'src_reg' registers into x64 opcode 'byte' */
static inline __u8 add_2reg(__u8 byte, __u32 dst_reg, __u32 src_reg)
{
...
static int do_jit(struct bpf_prog *bpf_prog, int *addrs, __u8 *image,
        int oldproglen, struct jit_context *ctx)
{
    ...
    for (i = 0; i < insn_cnt; i++, insn++) {
        ...
        __u32 dst_reg = insn->dst_reg;
        __u32 src_reg = insn->src_reg;
        __u8 b1 = 0, b2 = 0, b3 = 0;
        __s64 jmp_offset;
        __u8 jmp_cond;
        int ilen;
        ...
        switch (insn->code) {
            ...
            case BPF_LD | BPF_IMM | BPF_DW:
                if (insn[1].code != 0 || insn[1].src_reg != 0 ||
                    ...
                    printf("invalid BPF_LD_IMM64 insn\n");
                    return -EINVAL;
                }
...
ldx:			/* if insn->off == 0 we can save one extra byte, but
                ...
                /* STX XADD: lock *(__u32*)(dst_reg + off) += src_reg */
...
/*Stop supporting these instructions*/
        ...
        if (image) {
            if (unlikely(proglen + ilen > oldproglen)) {
                printf("bpf_jit_compile fatal error\n");
                return -EFAULT;
            }
