...
lttng create
lttng enable-event -u retlif_tp:tpfilt --filter '{% for num in range(n) %}(var{{ num }} == "str{{ num }}") && {% endfor %} (lyf == 42)'
lttng start
...
lttng create
lttng enable-event -u retlif_tp:tpfilt --filter '{% for num in range(n) %}(var{{ num }} == "str{{ num }}") && {% endfor %} (lyf == 43)'
lttng start
for i in `seq 1 $NR_TIMES`;
do
    clean=$clean$(./retlif-lttng CLEAN | head -n 1)","
